...
{
    // at the end we need to return the smallest element
    ...
    Node *coolNode = nullptr;

    coolNode = kthSmallestHelper(currNode -> left, counter, k);
    if(*counter == k)
    {
        return coolNode;
    }
    ...
    coolNode = kthSmallestHelper(currNode->right, counter, k);
    if(*counter == k)
    {
        return coolNode;
    }
